...
public:
    ...
    Conv2D(const std::shared_ptr<Tensor<D, R>> &a,
           ...
           int padding,
           int stride)
            : CNode<D, R>(Utils::removeOption<std::shared_ptr<CNodeBase>>({a->gradFn, filter->gradFn, bias->gradFn}),
                          result),
              a(a->eTensor),
     ...
     * result (h', w', c', n)
     * */
    static std::shared_ptr<Tensor<D, R>> conv2d(const std::shared_ptr<Tensor<D, R>> &a,
                                                const std::shared_ptr<Tensor<D, R>> &filter,
                                                const std::shared_ptr<Tensor<D, 1>> &bias,
                                                int padding,
                                                int stride) {
    ...
    void computeGradients() override {
        ...
        if (ca.has_value()) {
            int temp = (ca.value()->shape[1] + filter->dimension(1) - stride * (CNode<D, R>::grad->dimension(1) - 1) -
                        2);
            int backPadding = temp / 2;
            int additionalBRPadding = temp % 2;
            Eigen::Tensor<D, R> backFilter(filter->dimension(3), filter->dimension(1), filter->dimension(2),
                                           filter->dimension(0));
            backFilter.device(myDevice) = filter->reverse(Eigen::array<bool, R>{false, true, true, false}).shuffle(
                    Eigen::array<int, 4>{3, 1, 2, 0});
            ca.value()->addGrad(
                    myConvolution(*CNode<D, R>::grad, backFilter, backPadding, 1, stride, additionalBRPadding));
        }
        if (cfilter.has_value()) {
            Eigen::array<long, R + 1> reshape{1, a->dimension(0), a->dimension(1), a->dimension(2), a->dimension(3)};
            Eigen::array<long, R - 1> reshape2{a->dimension(1) * a->dimension(2),
                                               a->dimension(0) * filter->dimension(1) * filter->dimension(2),
                                               a->dimension(3)};
            Eigen::array<long, R + 1> reshape3{filter->dimension(0), filter->dimension(1), filter->dimension(2),
                                               a->dimension(3), filter->dimension(3)};


            auto avol = a->reshape(reshape).extract_volume_patches(1, a->dimension(1), a->dimension(2), 1, 1, 1, 1, 1,
                                                                   1, 0, 0, padding, padding, padding, padding);

            auto im2col = avol.reshape(reshape2);

            if (stride == 1) {
                auto i2cFilter = CNode<D, R>::grad->sum(Eigen::array<int, 1>{3}).reshape(
                        Eigen::array<long, 2>{CNode<D, R>::grad->dimension(0), reshape2[0]}).eval();
                auto conv = im2col.contract(i2cFilter,
                                            Eigen::array<Eigen::IndexPair<int>, 1>{Eigen::IndexPair<int>(0, 1)});
                ...
                Eigen::Tensor<D, R - 1> summedGrad(CNode<D, R>::grad->dimension(0), CNode<D, R>::grad->dimension(1),
                                                   CNode<D, R>::grad->dimension(2));
...
private:
    ...
    static auto
    myConvolution(const OtherDerived &a, const OtherDerived2 &filter, int padding, int stride = 1, int dialation = 1,
                  int additionalBRPadding = 0) {

        auto patches = a.extract_image_patches(filter.dimension(1), filter.dimension(2), stride, stride, 1, 1,
                                               dialation, dialation, padding, padding + additionalBRPadding, padding,
                                               padding + additionalBRPadding, 0);

        int sheight = (dialation * (a.dimension(1) - 1) + 1 - filter.dimension(1) + 2 * padding + additionalBRPadding) /
                      stride + 1;
        int swidth = (dialation * (a.dimension(2) - 1) + 1 - filter.dimension(2) + 2 * padding + additionalBRPadding) /
                     stride + 1;
