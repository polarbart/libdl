...
public:
    ...
    static std::shared_ptr<Tensor<D, RA - RB>> sum(
        ...
        auto result = std::make_shared<Tensor<D, RA - RB>>(x->eTensor->sum(axes), newShape);
        if (x->needsGradient())
            result->setGradFn(std::make_shared<SumAlongAxes<D, RA, RB>>(x->gradFn, result, axes, x->eTensor->dimensions()));
        return result;
    }
