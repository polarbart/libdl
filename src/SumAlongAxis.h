...
public:
    SumAlongAxis(std::optional<std::shared_ptr<CNode<D, R + 1>>> a,
    ...
    static std::shared_ptr<Tensor<D, R - 1>> sum(std::shared_ptr<Tensor<D, R>> a, int axis) {
        ...
        if (a->needsGradient())
            result->setGradFn(std::make_shared<SumAlongAxis<D, R - 1>>(a->gradFn, shape, result, axis, a->eTensor->dimension(axis)));
        return result;
    }
