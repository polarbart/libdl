...
public:
     ...
     * result (h', w', c', n)
     * */
    static std::shared_ptr<Tensor<D, R>> conv2d(const std::shared_ptr<Tensor<D, R>> &a, const std::shared_ptr<Tensor<D, R>> &filter, const std::shared_ptr<Tensor<D, 1>> &bias, PaddingType padding) {

        std::array<long, R> newDims {
                padding == PaddingType::SAME ? a->eTensor->dimension(0) : (a->eTensor->dimension(0) - filter->eTensor->dimension(0) / 2 * 2),
                padding == PaddingType::SAME ? a->eTensor->dimension(1) : (a->eTensor->dimension(1) - filter->eTensor->dimension(1) / 2 * 2),
                filter->eTensor->dimension(3),
                a->eTensor->dimension(3),
        };

        // TODO: implement padding, see if cfilter backward works with my conv, optional padding for back ca

        auto r = myConvolution(*a->eTensor, *filter->eTensor, padding, newDims[0], newDims[1]);
    ...
    void computeGradients() override {
        ...
        if (cfilter.has_value()) {
            ...
            auto shuffled = a->shuffle(Eigen::array<int, 4> {2, 0, 1, 3}).eval();
            auto patches = shuffled.extract_image_patches(CNode<D, R>::grad->dimension(0), CNode<D, R>::grad->dimension(1));
            ...
            cfilter.value()->addGrad(conv.reshape(reshape2).mean(Eigen::array<int, 1> {4}));
        }
        if (cbias.has_value()) {
...
private:
    ...
    static auto myConvolution(const OtherDerived &a, const OtherDerived2 &filter, PaddingType padding, int newWidth, int newHeight) {

        Eigen::array<ptrdiff_t, R> patchDims{filter.dimension(0), filter.dimension(1), a.dimension(2), a.dimension(3)};

        auto shuffled = a.shuffle(Eigen::array<int, 4> {2, 0, 1, 3}).eval();
        auto patches = shuffled.extract_image_patches(filter.dimension(0), filter.dimension(1), 1, 1, 1, 1, padding);
