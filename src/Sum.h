...
class Sum : public CNode<D, R> {
public:
    Sum(std::optional<std::shared_ptr<CNode<D, R + 1>>> a, std::shared_ptr<Tensor<D, R>> r, int axis, long size)
            : CNode<D, R>(Utils::removeOption<std::shared_ptr<CNodeBase>>({a}), r), a(a), axis(axis), size(size) {}

    static std::shared_ptr<Tensor<D, R - 1>> sum(std::shared_ptr<Tensor<D, R>> a, int axis) {
        ...
        t = a->eTensor.sum(Eigen::array<int, 1>{axis});
        auto result = std::make_shared<Tensor<D, R - 1>>(data, shape);
        if (a->needsGradient())
            result->setGradFn(std::make_shared<Sum<D, R - 1>>(a->gradFn, result, axis, a->eTensor.dimension(axis)));
        return result;
    }
